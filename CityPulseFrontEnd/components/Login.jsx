...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ==========================================
      //  🚀 التعديل الذكي لحفظ البيانات والتوجيه
      // ==========================================
      if (response.data.token) {
        localStorage.setItem('token', response.data.token);
        
        // حفظ حقل الـ user كاملاً للتعرف عليه في جميع الواجهات لاحقاً
      ...
      Swal.fire({
        icon: 'success',
        title: 'Welcome Back!',
        text: `Login Successful! Welcome ${response.data.user.name}.`,
        confirmButtonColor: '#04332D',
        customClass: { popup: 'rounded-xl' }
      });

      // 🔄 التوجيه الشرطي حسب الصلاحية
      const userRole = response.data.user?.role;
      if (userRole === 'admin') {
        navigate('/dashboard'); // الأدمن يروح للوحة التحكم الجديدة فوراً
      } else {
        navigate('/report-issue'); // أي مستخدم آخر يروح لصفحة البلاغات
      }
